...
#' @param report A data frame containing the molecular report data generated by Formularity (Tolic et al, 2017, Analytical Chemistry)
#' @param ver The version of the ftmsRanalysis package to use. Supports "1.1.0" and "1.2.0". Default is "1.2.0".
#' @return A data frame of the molecular traits
#' @rdname molTrait
#' @examples
#' \dontrun{
#' report = read.csv(file = list.files(path = ".", pattern = "*Report.csv", full.names = T))
#' mol.trait = molTrait(report, ver = "1.2.0")}
#' @export
    
molTrait <- function(report, ver = "1.2.0"){
  
  check_suggested("ftmsRanalysis", "molTrait") # package found at https://github.com/EMSL-Computing/ftmsRanalysis
  as.peakData <- getExportedValue("ftmsRanalysis", "as.peakData")
  compound_calcs <- getExportedValue("ftmsRanalysis", "compound_calcs")
  assign_class <- getExportedValue("ftmsRanalysis", "assign_class")
  ...
  data = peak_icr$e_data
  mol = peak_icr$e_meta
  
  mol$Mass = as.numeric(mol$Mass)
  ...
  # Merging Y_met into mol data
  df = data.frame(formula = df$formula, Y_met = df$stoichMet_O2_donor)
